...
class M_data extends CI_Model{
    ...
    public function filter($dari, $sampai) {
		return $this->db->query("select * from tb_transaksi join tb_user on tb_transaksi.id_user = tb_user.id_user join tb_member on tb_transaksi.id_member = tb_member.id_member where tgl_bayar >= '$dari' and tgl_bayar <= '$sampai' order by id desc");
	}
    ...
    public function get_row_transaksi($n, $start)
    {
        ...
        return $this->db->from('tb_transaksi')
        ->join('tb_member', 'tb_member.id_member=tb_transaksi.id_member')
        // ->join('tb_paket', 'tb_paket.harga=tb_transaksi.harga_awal')
        ->get()
        ->result();
    ...
    public function get_full_records($where){
		...
		// $this->db->join('tb_paket', 'tb_paket.harga = tb_transaksi.harga_awal');
		$this->db->where($where);
		return $this->db->get();
	}
}